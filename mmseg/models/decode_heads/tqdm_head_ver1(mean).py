...
class tqdmHead(BaseDecodeHead):
    ...
    def forward(self, feats, texts, img_metas, return_mask_features=False, get_similarity=False): ### need texts for pixel_decoder !
        ...
        #$#$#$#$#$#$#$ start
        M = self.num_queries - self.num_classes
        cls_pred = torch.cat([
            cls_pred[:, :-(M+1), :],  # 첫 번째 부분: [2, 24-(M), 21]
            torch.mean(cls_pred[:, -(M+1) :, :], dim=1, keepdim=True)  # 두 번째 부분: M개의 채널 합침
        ], dim=1)
        mask_pred = torch.cat([
            mask_pred[:, :-(M+1), :, :],  # 첫 번째 부분: [2, 24-(M), 128, 128]
            torch.mean(mask_pred[:, -(M+1) :, :, :], dim=1, keepdim=True)  # 두 번째 부분: M개의 채널 합침
        ], dim=1)   
        #$#$#$#$#$#$#$ end
        ...
        for i in range(self.num_transformer_decoder_layers):
            ...
            #$#$#$#$#$#$#$ start
            # learnable query로 늘어난 cls_pred와 mask_pred를 줄이기            
            cls_pred = torch.cat([
                cls_pred[:, :-(M+1), :],  # 첫 번째 부분: [2, 24-(M), 21]
                torch.mean(cls_pred[:, -(M+1) :, :], dim=1, keepdim=True)  # 두 번째 부분: M개의 채널 합침
            ], dim=1)
