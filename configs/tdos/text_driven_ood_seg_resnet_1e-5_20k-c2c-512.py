_base_ = [
    ...
    '../_base_/datasets/city2city-512_resnet.py',]
...
model = dict(
    ...
    decode_head=dict(
        ...
        loss_cls=dict(
            ...
            loss_weight=2.0,
            reduction='mean',
            class_weight=[1.0] * 19 + [1.0] + [0.1]), # 일반 19개 , ood 1 개, 배경 1개 #$#$#$#$#$#$
        loss_mask=dict(
            type='CrossEntropyLoss',
            use_sigmoid=True,
            reduction='mean',
    ...
    # test_cfg=dict(mode='slide', crop_size=(224, 224), stride=(150, 150))
)
...
evaluation = dict(interval=500, metric='mIoU')  # 50 iter마다 평가 수행

work_dir = './work_dirs_d_clip_resnet_bipartite_224_freeze_surgery'
